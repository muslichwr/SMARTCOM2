        ...
        <div class="w-full mt-3">
            <div class="bg-white shadow-md rounded-lg">
                <div class="bg-gray-200 px-4 py-3 rounded-t-lg">
                    <h3 class="text-lg font-semibold">Edit Latihan</h3>
                    <a href="{{ url('admin/latihan') }}" class="flex gap-1 items-center bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 text-sm rounded float-right mt-5">
                ...
                <div class="p-4">
                    ...
                    <form action="{{ url('admin/latihan/' . $latihan->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="tab-content mt-2" id="myTabContent">
                            <div class="tab-pane fade border p-3 show active" id="home-tab-pane" role="tabpanel"
                                    ...
                                    <select name="materi_id"
                                        ...
                                        @foreach ($materis as $materi)
                                            <option value="{{ $materi->id }}"
                                                {{ $materi->id == $latihan->materi_id ? 'selected' : '' }}>
                                                {{ $materi->judul }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-4">
                                    <label class="block mb-1">Judul Latihan</label>
                                    <input type="text" name="judulLatihan" value="{{ $latihan->judulLatihan }}"
