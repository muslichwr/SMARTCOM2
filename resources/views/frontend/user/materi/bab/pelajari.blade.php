    ...
    <main class="mt-3 pt-8 pb-16 lg:pt-16 lg:pb-24 bg-white dark:bg-gray-900 antialiased">
        <div class="flex justify-between px-4 mx-auto max-w-screen-xl">
            <article class="mx-auto w-full max-w-2xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
                ...
                <form action="{{ url('user/materi/' . $bab->slug . '/selesaiMateri') }}" class="mb-6" method="POST" id="materiForm">
                    @csrf
                    @method('POST')
                    <div class="flex items-center justify-between">
                        ...
                        @if ($bab->status == 0 && (!$babAttempts || $babAttempts->count() == 0 || $babAttempts[0]->status != 1))
                            ...
                            <p id="timerText" class="text-sm text-gray-600 dark:text-white">Waktu tunggu: 8 menit</p>
                        @else
                            @if ($nextBab)
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const timerInterval = setInterval(function() {
                ...
                timerText.textContent = `Waktu tunggu: ${minutes} menit ${seconds} detik`;
                
                if (countdownTime <= 0) {
                    clearInterval(timerInterval);
                    selesaiButton.disabled = false;  // Enable button
                    timerText.textContent = 'Anda dapat menyelesaikan materi sekarang!';
                } else {
                    countdownTime--;
                }
            }, 1000);
