    ...
    <div class="flex justify-between px-4 mx-auto max-w-screen-xl">
        <article class="mx-auto w-full max-w-2xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
            ...
            <div id="timer" class="mb-6">
                <p class="text-lg font-semibold text-gray-900 dark:text-white">Selesaikan Materi dalam waktu:</p>
                <div id="countdown" class="text-xl font-bold text-red-500">
                    08:00
                </div>
            </div>
            ...
            <form action="{{ url('user/materi/' . $bab->slug . '/selesaiMateri') }}" class="mb-6" method="POST" id="selesaiMateriForm">
                @csrf
                @method('POST')
                <div class="flex items-center justify-between">
                    ...
                    @if ($bab->status == 0 && (!$babAttempts || $babAttempts->count() == 0 || $babAttempts[0]->status != 1))
                        <button type="submit" id="selesaiButton"
                            class="inline-flex items-center py-2.5 px-4 text-xs font-medium text-center text-white bg-primary-700 rounded-lg focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-primary-800 hidden">
                            Selesaikan Materi
                        </button>
                    @else
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const totalMinutes = Math.floor(Math.random() * (10 - 8 + 1)) + 8; // Random timer between 8 and 10 minutes
        let timeRemaining = totalMinutes * 60; // Convert to seconds
        const countdownElement = document.getElementById('countdown');
        const selesaiButton = document.getElementById('selesaiButton');
        const selesaiForm = document.getElementById('selesaiMateriForm');
        ...
        const timerInterval = setInterval(function () {
            if (timeRemaining > 0) {
                ...
                selesaiButton.classList.remove('hidden'); // Show the selesai button
            }
        }, 1000);

        // Prevent the form submission after the timer ends
        selesaiForm.addEventListener('submit', function (event) {
